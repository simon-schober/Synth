...
void SinWaveDataCallback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount)
{
    ...
    for (ma_uint32 i = 0; i < frameCount; ++i) {
        float sample = 0.2f * sinf(2.0f * 3.14159265359f * g_audioState.phase);
        for (ma_uint32 ch = 0; ch < pDevice->playback.channels; ++ch) {
            *pOutputF32++ = sample;
        }
        // Phase increment is proportional to frequency
        g_audioState.phase += g_audioState.frequency / (float)DEVICE_SAMPLE_RATE;
        // Keep phase wrapped to prevent overflow
        while (g_audioState.phase >= 1.0f) g_audioState.phase -= 1.0f;
    }
    (void)pInput;
}
...
bool InitializeAudio(ma_device& device)
{
    ...
    config.dataCallback = SinWaveDataCallback;
